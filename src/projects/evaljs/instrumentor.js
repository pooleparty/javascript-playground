...
/* global esprima, escodegen, window, recast */
(function exec(isNode) {
  let SYNTAX;
  let nodeType;
  const ESP = isNode ? require('esprima') : esprima;
  const ESPGEN = isNode ? require('escodegen') : escodegen; // TODO - package as dependenc
  const RECAST = isNode ? require('recast') : recast; // TODO - package as dependenc
  const crypto = isNode ? require('crypto') : null;
  const LEADER_WRAP = '(function () { ';
  const TRAILER_WRAP = '\n}());';
  ...
  astgen = {
    thisExpression() {
      return { type: SYNTAX.ThisExpression.name };
    },
    variable(name) {
    ...
    statement(contents) {
      return { type: SYNTAX.ExpressionStatement.name, expression: contents };
    },
    property(key, value) {
      return {
        type: SYNTAX.Property.name,
        key,
        value,
        // computed: false,
        // kind: 'init',
    ...
    objectExpression(properties) {
      return { type: SYNTAX.ObjectExpression.name, properties };
    },
    arrayExpression(elements) {
      return { type: SYNTAX.ArrayExpression.name, elements };
    },
    callExpression(callee, ...args) {
      return {
        type: SYNTAX.CallExpression.name,
        callee,
        arguments: args,
      };
    },
    functionExpression(id, params, body) {
      return {
        type: SYNTAX.FunctionExpression.name,
        id,
        params,
        body,
      };
    ...
    blockStatement(body) {
      return {
        type: SYNTAX.BlockStatement.name,
        body,
      };
    },
    dot(obj, field) {
    ...
    sequence(one, two) {
      return { type: SYNTAX.SequenceExpression.name, expressions: [one, two] };
    },
  ...
  function defaultWalker(node, walker) {
    ...
    if (isArray(walkerFn)) {
      for (walkFnIndex = 0; walkFnIndex < walkerFn.length; walkFnIndex += 1) {
        ...
        if (ret.type !== type && !isLast) {
          throw new Error(
            `Only the last walker is allowed to change the node type: [type was: ${type} ]`,
          );
        }
      }
    } else if (walkerFn) {
    ...
    for (i = 0; i < children.length; i += 1) {
      ...
      if (childNode && !childNode.skipWalk) {
        pathElement = { node, property: childType };
        if (isArray(childNode)) {
          ...
          for (j = 0; j < childNode.length; j += 1) {
            ...
            if (isArray(assignNode.append)) {
              pushAll(childArray, assignNode.append);
              delete assignNode.append;
            }
          }
  ...
  Walker.prototype = {
    ...
    apply(node, walkFn, pathElement) {
      let ret, i, seq, prefix;
    ...
    ancestor(n) {
      return this.path.length > n - 1
        ? this.path[this.path.length - n]
        : /* istanbul ignore next: guard */ null;
    },
  ...
  Instrumenter.prototype = {
    ...
    instrumentSync(code, filename, options) {
      ...
      if (code.charAt(0) === '#') {
        // shebang, 'comment' it out, won't affect syntax tree locations for things we care about
        code = `//${code}`;
      }
      ...
      return this.instrumentASTSync(program, filename, code, options);
    },
    filterHints(comments) {
      let ret = [],
        i,
        comment,
        groups;
    ...
    extractCurrentHint(node) {
      if (!node.range) {
        return;
      }
      let i = this.currentState.lastHintPosition + 1,
     ...
     */
    instrumentASTSync(program, filename, originalCode, opts) {
      const options = opts || {
        wrapInIFFE: false,
      };
      let usingStrict = false,
        codegenOptions,
        generated,
        preamble,
        lineCount,
      ...
      codegenOptions = this.opts.codeGenerationOptions || {
        format: { compact: !this.opts.noCompact },
      };
      ...
      if (options.wrapInIFFE) {
        generated = this.wrapInIFFE(generated);
      }

      return `${preamble}\n${generated}\n`;
    },
    wrapInIFFE(code) {
      const iffe = `(function iffe() { return %CODE%; })();`.replace(/%CODE%/g, code);
      return iffe;
    },
    /**g
    ...
    fixColumnPositions(coverState) {
      let offset = LEADER_WRAP.length,
        fixer = function(loc) {
          if (loc.start.line === 1) {
    ...
    getPreamble(sourceCode, emitUseStrict) {
      let varName = this.opts.coverageVariable || '__coverage__',
      ...
      coverState = this.opts.debug
        ? JSON.stringify(this.coverState, undefined, 4)
        : JSON.stringify(this.coverState);
      code = [
        '%STRICT%',
    ...
    arrowBlockConverter(node) {
      let retStatement;
      if (node.expression) {
        // turn expression nodes into a block with a return statement
    ...
    skipInit(node /* , walker */) {
      if (node.init) {
        node.init.skipWalk = true;
      }
    },

    skipLeft(node /* , walker */) {
      node.left.skipWalk = true;
    },

    isUseStrictExpression(node) {
      return (
        node &&
        node.type === SYNTAX.ExpressionStatement.name &&
        node.expression &&
        node.expression.type === SYNTAX.Literal.name &&
        node.expression.value === 'use strict'
      );
    },

    maybeSkipNode(node, type) {
      let alreadyIgnoring = !!this.currentState.ignoring,
    ...
    coverMetaProperty(node /* , walker */) {
      node.skipSelf = true;
    },

    coverStatement(node, walker) {
      let sName, emitStatement, parent, grandParent;
      ...
      if (this.isUseStrictExpression(node)) {
        ...
        if (grandParent) {
          if (
            (grandParent.node.type === SYNTAX.FunctionExpression.name ||
      ...
      if (node.type === SYNTAX.FunctionDeclaration.name) {
        ...
        // incrStatementCount = astgen.statement(astgen.postIncrement(astgen.subscript(
        //   astgen.dot(astgen.variable(this.currentState.trackerVar), astgen.variable('s')),
        //   astgen.stringLiteral(sName),
        // )));
        // incrStatementCount = astgen.statement(
        //   astgen.subscript(
        //     astgen.dot(astgen.variable('console'), astgen.variable('log')),
        //     astgen.stringLiteral(sName),
        //   ),
        // );

        const callee = astgen.dot(astgen.thisExpression(), astgen.variable('emit'));

        const properties = [];
        const arg1 = astgen.stringLiteral('NODE_STEP');
        const astArg = astgen.stringLiteral(JSON.stringify(node.type));
        properties.push(astgen.property(astgen.variable('nodeType'), astArg));
        const codeArg = astgen.stringLiteral(RECAST.print(node).code);
        properties.push(astgen.property(astgen.variable('code'), codeArg));

        const variables = [];
        if (node.type === SYNTAX.VariableDeclaration.name) {
          let { declarations } = node;
          declarations = declarations.filter(d => d.type === SYNTAX.VariableDeclarator.name);
          declarations.forEach(d => {
            const {
              id: { name },
            } = d;
            variables.push(astgen.property(astgen.variable(name), astgen.variable(name)));
          });
        } else if (node.type === SYNTAX.ExpressionStatement.name) {
          const {
            expression: { left },
          } = node;
          let name;
          // TODO: support CallExpressions
          if (left) {
            switch (left.type) {
              case SYNTAX.MemberExpression.name:
        ...
        properties.push(
          astgen.property(astgen.variable('variables'), astgen.objectExpression(variables)),
        );

        emitStatement = astgen.statement(
          astgen.callExpression(callee, arg1, astgen.objectExpression(properties)),
        );

        this.spliceEnd(emitStatement, node, walker);
      }
    },

    coverExport(node, walker) {
      let sName, incrStatementCount;
    ...
    splice(statements, node, walker) {
      const targetNode = walker.isLabeled() ? walker.parent().node : node;
      targetNode.prepend = targetNode.prepend || [];
      pushAll(targetNode.prepend, statements);
    },

    spliceEnd(statements, node, walker) {
      const targetNode = walker.isLabeled() ? walker.parent().node : node;
      targetNode.append = targetNode.append || [];
      pushAll(targetNode.append, statements);
    },

    functionName(node, line, location) {
      this.currentState.func += 1;
      let id = this.currentState.func,
        ignoring = !!this.currentState.ignoring,
        name = node.id ? node.id.name : `(anonymous_${id})`,
        clone = function(attr) {
          const obj = location[attr] || /* istanbul ignore next */ {};
    ...
    branchName(type, startLine, pathLocations) {
      let bName,
        paths = [],
        locations = [],
        i,
      ...
      this.coverState.branchMap[bName] = { line: startLine, type, locations };
      return bName;
    },

    branchIncrementExprAst(varName, branchIndex, down) {
      const ret = astgen.postIncrement(
        astgen.subscript(
          astgen.subscript(
    ...
    branchLocationFor(name, index) {
      return this.coverState.branchMap[name].locations[index];
    },

    switchBranchInjector(node, walker) {
    ...
    switchCaseInjector(node) {
      const location = node.branchLocation;
    ...
    conditionalBranchInjector(node, walker) {
      let bName = this.branchName(
          'cond-expr',
          walker.startLineForNode(node),
          this.locationsForNodes([node.consequent, node.alternate]),
        ),
    ...
    logicalExpressionBranchInjector(node, walker) {
      ...
      bName = this.branchName(
        'binary-expr',
        walker.startLineForNode(node),
        this.locationsForNodes(leaves.map(item => item.node)),
      );
      for (i = 0; i < leaves.length; i += 1) {
        tuple = leaves[i];
        tuple.parent[tuple.property] = astgen.sequence(
          this.branchIncrementExprAst(bName, i),
          tuple.node,
        );
    ...
    findLeaves(node, accumulator, parent, property) {
      if (node.type === SYNTAX.LogicalExpression.name) {
        ...
        accumulator.push({ node, parent, property });
      }
    },
    maybeAddType(node /* , walker */) {
